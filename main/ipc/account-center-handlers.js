...
function registerAccountCenterIpc(ipcMain,service){
 ...
 for(const [name,fn] of Object.entries({snapshot:()=>service.snapshot(),check:p=>service.check(p.id),open:p=>service.open(p.id),login:p=>service.login(p.id),release:p=>service.release(p.id)})){
