...
add('common','双 Agent 通用约束','主流程','双席位每轮消息及阶段派工追加；下面阶段正文不重复计入这段','core/dev-file-workflow.js · common',F.common(meeting,dir));
for(const [phase,n,title] of [['kickoff',0,'开题'],['build',1,'首次实现'],['build',2,'返工实现'],['merge',1,'验证与合并']]){
  const prompt=F.phasePrompt(meeting,dir,F.spec(phase,n)).slice(F.common(meeting,dir).length).trim();
...
add('solo-common','单 Agent 通用约束','主流程','本次新增：单 Agent 群聊每轮追加','core/dev-file-workflow.js · soloCommon',F.soloCommon({...meeting,serialWorkflow:{...meeting.serialWorkflow,soloDevelopment:true}}),true);
add('independent','独立开工','主流程','本次新增：单 Agent 点击按钮预填，检查后发送；同一位完成实现、验证、合并','core/dev-file-workflow.js · independentPrompt',F.independentPrompt(),true);
...
for(const id of ['dev-task','dev-task-solo']){
  const c=W.createTemplateConfig(id,[{memberId:'m1'},{memberId:'m2'}],{devPhase:'build'});
  c.stepConfigs.forEach((s,i)=>add(id+'-'+i,s.name,'旧版兼容',id==='dev-task-solo'?'旧极简配置；本次从创建页移除，历史群聊保留':'旧版双席位循环；新版创建不使用','renderer/workflow-templates.js · '+id+'/'+i,s.prompt));
}
...
</style></head><body><header><small>AI HUB / PROMPT WORKBENCH / 2026.09.10</small><h1>先把流程变简单，再把 prompt 写短</h1><p>当前主流程 <b>7 项</b>，本次增加单 Agent 通用约束与「独立开工」后为 <b>9 项</b>。按可单独维护的文本块统计；共享约束只计一次，首次实现与返工分开。不是每轮都会发送全部内容。</p><div class="stats" id="stats"></div><p>改动：开发排第一并默认选中 · 默认选择已有路径 · 删除起手卡片 · 一人负责实现与合并，两人按先后分工。</p></header>
...
<main><aside><div class="delta"><b>建议从这 3 处下手</b><p>1. 双 Agent 通用约束重复最多。<br>2. 验证与合并正文最长。<br>3. 独立开工可以保留为短指令。</p></div><nav id="nav"></nav><details><summary>统计与修改边界</summary><div class="note">本页完整列出主流程、8 个职责帽、旧版开发协议，以及可选通用工作流。{{变量}} 表示运行时填入的路径或任务。共享聊天包装、英雄人格、模型系统指令和项目 AGENTS/合同属于其他层，不计入开发预置数。<br><br>这里修改的是本地审阅草稿，不会直接改生产 Hub。导出文件交回后才能应用。空修改稿表示建议清空该项，不自动删除流程功能。<br><br>单 Agent 用同一会话自审，双 Agent 合并位独立验证；旧模板为兼容历史房间保留。</div></details><div class="note" id="provenance"></div></aside><section><div id="count" class="note"></div><div id="cards"></div></section></main><div id="toast" role="status"></div>
