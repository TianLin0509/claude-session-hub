...
function protocol() {
  return D.protocol() + '\n\n' + state.rounds.map((r,i)=>`${i+1}. ${r.name}\n${r.members.map(name).join(' / ')}\n${r.prompt}`).join('\n\n');
}
function render() {
  const file = state.kind === 'file';
  modal.querySelector('#wf-body').innerHTML = `<div class="wf-templatebar"><small>从模板开始</small>${[...S.PRESETS,{id:'custom',name:'＋ 自定义',minMembers:1}].map(p=>`<button class="wf-preset${state.presetId===p.id?' selected':''}" data-wf="task-preset" data-task-preset="${p.id}" ${members.length<p.minMembers?'disabled':''}>${p.name}</button>`).join('')}<span class="wf-budget">最多执行 <b>6</b> 轮 · 返工计入</span></div><div class="wf-summary"><span>${state.rounds.length} 个预设轮次 · 每轮 1–3 位 Agent · 同轮每位成员交付后接续 <em id="wf-dirty">${dirty?'已修改':''}</em></span><span class="wf-tag">文件交付 · 可恢复</span>${file?'':`<button data-wf="toggle" role="switch" aria-checked="${state.enabled}">${state.enabled?'已启用':'未启用'}</button>`}</div><div class="wf-scroll"><div class="wf-steps">${state.rounds.map((r,i)=>`<article class="wf-step-row" data-round="${i}"><header class="wf-round-header"><span class="wf-step-index">${String(i+1).padStart(2,'0')}</span><input class="wf-step-name" maxlength="80" aria-label="第 ${i+1} 轮名称" data-wf-step-name="${i}" value="${esc(r.name)}"><div class="wf-round-tools"><button data-wf="move" data-step="${i}" data-delta="-1" aria-label="上移第 ${i+1} 轮" ${file||i===0?'disabled':''}>↑</button><button data-wf="move" data-step="${i}" data-delta="1" aria-label="下移第 ${i+1} 轮" ${file||i===state.rounds.length-1?'disabled':''}>↓</button><button data-wf="remove" data-step="${i}" aria-label="删除第 ${i+1} 轮" ${file||state.rounds.length===1?'disabled':''}>×</button></div></header><div class="wf-member-chips"><small>参与者</small>${members.map(m=>`<button class="wf-member-chip${r.members.includes(m.memberId)?' selected':''}" data-wf="chip" data-step="${i}" data-member="${esc(m.memberId)}" aria-pressed="${r.members.includes(m.memberId)}"><img src="assets/ai-logos/${esc(String(m.kind||'claude').replace(/-resume$/,''))}.svg" alt="">${esc(m.title||m.memberId)}</button>`).join('')}</div><label class="wf-prompt-label" for="wf-prompt-${i}">本轮共享 Prompt · 交付要求自动附加</label><textarea id="wf-prompt-${i}" class="wf-step-prompt" maxlength="16000" rows="4" data-wf-step-prompt="${i}">${esc(r.prompt)}</textarea><div class="wf-round-rule"><span>${r.members.length} 位参与 · 同一份指令 · 每人独立交付${file?' · '+esc(name(r.members[0]))+' 负责阶段结果':''}</span>${file?`<span>${i===2?'完成 → 结束 · 需返工 → 实现':'交付后 → 下一轮'}</span>`:`<label>交付后 <select data-after="${i}" aria-label="第 ${i+1} 轮交付后"><option value="next" ${r.after==='next'?'selected':''}>进入下一轮</option><option value="end" ${r.after==='end'?'selected':''}>结束流程</option></select></label>`}</div></article>`).join('')}</div><button class="wf-add" data-wf="add" ${file||state.rounds.length>=6?'disabled':''}>${file?'开发文件流保留三段接续结构；返工复用实现与评审配置':state.rounds.length>=6?'已达 6 个预设轮次':'＋ 添加一轮'}</button><div class="wf-protocolbar"><span>任务与前序交付随派工附加；超时或聊天结束不会跳到下一轮</span><button data-wf="protocol">查看交付规则</button></div><pre id="wf-protocol" hidden></pre><div id="wf-preview" hidden></div></div>`;
  message(context?.legacyTaskFiles ? '此群已有旧版交付文件，本次保存保留原协议；新任务请新建群聊使用逐成员交付。' : state.legacyProtocol ? '此群仍使用旧版执行协议；保留原设置继续使用，选择新模板后才会转换。' : '保存后，本群的新任务采用逐成员文件交付。');
}
function preview() {
...
async function save() {
  try {
    ...
    if (typeof onSave === 'function') await onSave(S.toDeliveryConfig(sourceConfig,state,members.map(m=>m.memberId)),structuredClone(state));
