...
(function () {
  ...
  const state = { opened:false, loading:false, request:0, epoch:null, sequence:-1,
    ...
    filter:'all', search:'', project:'', mode:'tasks', sort:'updated-desc', projectLimits:new Map(), historyLimits:new Map(), page:0, buffered:[], error:'', updatedAt:0 };
  ...
  function saveView(){try{localStorage.setItem('hub.devWorkbench.view',JSON.stringify({mode:state.mode,sort:state.sort}));}catch(error){state.error='视图偏好未能保存：'+error.message;}}
  ...
  function buildSkeleton(){
    ...
    root.innerHTML=`<header class="devb-head"><div><h2>开发群聊工作台</h2><p class="devb-subtitle">一个开发群聊对应一个任务。进展随群聊汇报同步，操作回到同一条流程。</p></div><div class="devb-head-actions">${button('reload','重新载入','id="devb-refresh"')}${button('create','新建开发群聊','class="devb-primary"')}</div></header>
      <div class="devb-viewbar"><div class="devb-tabs" role="tablist" aria-label="工作台视图">${[['tasks','任务'],['projects','项目']].map(([mode,label])=>button('mode',label,`id="devb-tab-${mode}" role="tab" aria-controls="devb-list" data-mode="${mode}" aria-selected="${state.mode===mode}" tabindex="${state.mode===mode?'0':'-1'}"`)).join('')}</div><span class="devb-order-hint">置顶优先 · 其余按时间排列 · 置底最后</span><select id="devb-sort" aria-label="时间排序"><option value="updated-desc">最近更新优先</option><option value="created-desc">最近创建优先</option><option value="updated-asc">较早更新优先</option><option value="created-asc">较早创建优先</option></select></div>
      ...
      <div class="devb-summary"><span id="devb-status" aria-live="polite">准备载入…</span><span id="devb-sync">群聊汇报自动推送</span></div><div id="devb-banner" role="status"></div><div id="devb-list" role="tabpanel"><div class="devb-grid"></div><div class="devb-empty"></div><div class="devb-pager"></div></div>
    ...
    root.querySelector('#devb-sort').addEventListener('change',event=>{state.sort=event.target.value;state.page=0;saveView();render();});
  ...
  function rowHtml(row){
    ...
    const warning=row.feedError?'汇报载入失败：'+row.feedError:row.lastError||row.blockers||'';
    return `<article class="devb-row" data-mid="${esc(id)}" style="--devb-tone:${tone}"><div class="devb-row-main"><div class="devb-identity"><h3>${row.pinned?'<span class="devb-placement">置顶</span>':row.bottomed?'<span class="devb-placement bottom">置底</span>':''}${esc(text(row.title,240)||'未命名开发群聊')}</h3><div class="devb-project" title="${esc(row.workspace)}">${esc(Model.projectName(row))}</div><time class="devb-time" title="创建于 ${esc(timeText(row.createdAt))}">更新 ${esc(timeText(row.activityAt))}</time></div>
      <div class="devb-latest"><span class="devb-eyebrow">最新进展</span><p class="devb-progress">${esc(row.progress||(row.loading?'正在载入已有群聊汇报…':'尚未收到进展汇报。Agent 写出更新后会在这里显示。'))}</p><div class="devb-source">${esc(sourceText(row.progressSource)||'来源：本开发群聊')}${row.progressSource?.runId&&row.execution?.runId&&row.progressSource.runId!==row.execution.runId?' · 上一轮汇报；当前轮尚无更新':''}</div></div>
      ...
      <div class="devb-row-links">${button('open','进入群聊','class="devb-primary"')}${button('details',state.expanded.has(id)?'收起详情':'任务详情',`aria-expanded="${state.expanded.has(id)}"`)}<details class="devb-menu" data-detail-key="${esc(id)}-menu" ${state.expanded.has(id+'-menu')?'open':''}><summary aria-label="任务操作">•••</summary><div class="devb-menu-items">${button(row.pinned?'unpin':'pin',row.pinned?'取消置顶':'置顶',disabled)}${button(row.bottomed?'unbottom':'bottom',row.bottomed?'取消置底':'置底',disabled)}${actions.stop?button('stop','停止流程',disabled):''}${actions.resume?button('resume','恢复中断流程',disabled):''}${actions.takeover?button('takeover','手动接管',disabled):''}${actions.restore?button('restore','恢复自动设置',disabled):''}</div></details></div></div>
      ${warning?`<div class="devb-warning">${esc(warning)}</div>`:''}
      <div class="devb-details" ${state.expanded.has(id)?'':'hidden'}>${row.goal?`<div class="devb-goal"><strong>任务目标</strong>　${esc(row.goal)}</div>`:''}<dl><dt>实现</dt><dd>${esc(card.progress||'尚无最终交接汇报')}</dd><dt>自测</dt><dd>${esc(card.verified||'尚无自测汇报')}</dd><dt>风险</dt><dd>${esc(card.risk||'未报告风险')}</dd><dt>审核</dt><dd>${esc(review.decision==='pass'?'最近一次审核报告 PASS':review.decision==='fail'?'最近一次审核报告 FAIL':'尚无审核裁决')}</dd><dt>验证</dt><dd>${esc(review.verified||'尚无独立验证汇报')}</dd>${review.next?`<dt>下一步</dt><dd>${esc(review.next)}</dd>`:''}</dl><p class="devb-source">${esc(sourceText(row.review||row.card))}。席位汇报供你核对，审核通过不代表已经合并或发布。${row.truncated?'长汇报已节选，完整内容请查看群聊原文。':''}</p>${row.report?button('report','查看报告'):''}</div>
  ...
  function render(){
    ...
    try{
      ...
      const filtered=Model.sortRows(rows.filter(row=>(state.filter==='all'||bucket(row)===state.filter)&&(!state.project||Model.projectKey(row)===state.project)&&(!needle||[row.title,row.project,row.workspace,row.progress].join(' ').toLocaleLowerCase().includes(needle))),state.sort);
      const showingProjects=state.mode==='projects',items=showingProjects?Model.groupProjects(filtered,state.sort):filtered,pageSize=showingProjects?12:PAGE_SIZE;
      ...
      root.querySelector('.devb-order-hint').textContent=showingProjects?'按项目时间排列 · 当前任务优先展示 · 历史按需展开':'置顶优先 · 其余按时间排列 · 置底最后';
  ...
  async function handleAction(target){
    ...
    if(action==='mode'){state.mode=target.dataset.mode;state.page=0;saveView();render();listEl.scrollTop=0;return;}
