...
在左侧功能栏的记忆入口之后增加拼图图标「技能」。页面提供能力库、AI 覆盖对比、当前会话三个视图。调研依据与实现取舍见 [capability-center-research.md](capability-center-research.md)。
...
4. Codex 使用当前实例的 `skills/list`、`mcpServerStatus/list`。接口以本机 Codex 0.153.4 生成的 JSON schema 核对。官方文档将 `plugin/list` 标为不供生产客户端调用，本版移除此请求并明确会话插件状态未确认。技能已发现、MCP runtimeStatus 分开表述。缺失 runtimeStatus 不从工具数量推断连接成功。
...
- 列表直接展示简短用途与来源标签，插件名称的市场后缀移至原始信息折叠区。原始名称、说明和配置路径完整保留。
- 中文用途词典仅用于展示，不证明已安装、已启用或来源。自定义 MCP 的 `description` 可作为说明；无法识别的条目显示待补充。
- 创建来源独立于公共/用户/项目安装范围。插件市场和内置技能标为外部；`personal` 标为自建封装（不宣称组件全部原创）；技能可通过 `origin: self/external` 显式声明。单凭本地路径不能判为自建，混合来源显示差异。
- 详情可以编辑简述（180 字符以内）和来源，恢复自动识别。仅写 Hub 数据目录 `capability-notes/<id-sha256>.json`，独立条目原子发布并使缓存失效；不修改原生规则、技能正文、MCP 连接或插件开关。
- 来源筛选、搜索、覆盖表共用展示信息。当前会话只借用简述和来源，不借用目录状态替代原生回执。
- MCP 详情给出共享建议：禁用条目先确认启用意图；插件依赖先核对宿主适配；stdio 复用程序但各 Agent 各自启动进程；HTTP 可评估复用服务地址。`arena-research` 需要 Hub 的研究任务路由，不能直接复制当前任务凭据。此版本不批量写入 MCP 配置。

本页仅“补齐共享技能”执行新增目录链接，先预览、校验一次性计划、再执行并记录；简述与来源标注只写 Hub 展示信息，其余原生能力配置只读。不提供删除、启用 MCP、变更插件开关或自动合并同名技能。内置平台工具、未在本机配置登记的账户连接器，以及其它 Hub 未打开的项目不宣称完整发现。Codex 多版本缓存不能代替精确安装回执；需要到原生客户端核实。插件详情只关联已盘点的 Skill / MCP；未覆盖的其他组件明确提示。
