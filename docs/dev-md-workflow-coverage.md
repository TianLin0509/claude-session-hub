...
| E02 | I | `resolveLaunchDir` 退到存在的祖先并明说是落脚点；建房闸门对失效路径退回**所属项目根**，没有归属就退到**平铺工作根**；核实不过不开工也不绑 | U：3 条 + `unit-dev-workspace-guard.test.js` 4 条；I：「E02/I …」3 条 | 通过 | 隔离实例上验了：项目根核实不过 → 停在开题、状态给出具体原因、未核实路径一律不绑。建房侧：失效路径若还属于某个项目就退回项目根并放行；完全不属于任何项目才继续报「目录不存在」（这条是有意保留的边界，见下方未完成项）|
...
5. ~~失效目录且不属于任何项目仍硬报错~~ —— 2026-09-08 已按合并位的判断放宽：
   找不到所属项目时退到**平铺工作根**（受约束的有效目录，带项目库，定位说明接手），
   并把这个房标成「不在项目根上」。只有连工作根都拿不到才继续报「目录不存在」。
