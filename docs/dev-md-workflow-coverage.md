...
| D07 | U+I | 双击 / 旧请求晚到 / 状态损坏 | I：「D07/I …」6 条 | 通过 | 本轮修正了上一版的误报（曾在已完成态双击、两次都被拒也算过），并因此逼出一条真缺陷：状态记录被写坏时引擎原来照单全收 |
...
| ID | 层级 | 实现位置 | 脚本入口 | 结果 | 证据 / 说明 |
|---|---|---|---|---|---|
| E01 | I+L | `core/dev-project-locator.js` 唯一命中即自主继续；开题报告「项目根：」绑定 | U：`unit-dev-project-locator.test.js` 2 条 | 部分（U 通过，I/L 未执行） | 唯一命中的说明文本与绑定校验已验；真实 CLI 上「两位后续使用同一核实路径」未跑 |
| E02 | I | `resolveLaunchDir` 退到存在的祖先并明说是落脚点 | U：2 条 | 部分（U 通过，I 未执行） | 「不在错仓库写」目前靠 prompt 约束 + 不绑定未核实路径，未在隔离实例上重放 |
| E03 | U+I | 多候选只问一个问题；路径只是例子不当目标 | U：2 条 | 部分（U 通过，I 未执行） | — |
| E04 | I | worktree（.git 是文件）算有效现场；子目录能找到仓库根；任务目录与工作目录解耦 | U：4 条 | 部分（U 通过，I 未执行） | 「附件修正同步 Hub」结构上成立（任务目录只由 Hub 数据目录 + meetingId 决定），已由单测断言 |
