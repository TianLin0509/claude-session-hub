...
| E02 | I | `resolveLaunchDir` 退到存在的祖先并明说是落脚点；建房闸门对失效路径退回它**所属的项目根**；核实不过不开工也不绑 | U：3 条 + `unit-dev-workspace-guard.test.js` 2 条；I：「E02/I …」3 条 | 通过 | 隔离实例上验了：项目根核实不过 → 停在开题、状态给出具体原因、未核实路径一律不绑。建房侧：失效路径若还属于某个项目就退回项目根并放行；完全不属于任何项目才继续报「目录不存在」（这条是有意保留的边界，见下方未完成项）|
...
5. **失效目录且不属于任何项目**：建房仍然硬报「目录不存在」。任务书第七节要的是「在有效目录进入只读定位」，
   我只对「路径还属于某个项目」的情况做了兜底；退到一个随便什么存在的文件夹会造出开在非项目目录上的开发房，
   把失败推迟到第一步。这是有意保留的边界，需要维护者拍板是否要放宽。
