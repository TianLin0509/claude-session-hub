...
# Hub PackyAPI 多模型 Session 设计（GPT 5.4 high / Kimi K2.5 / Qwen 3.6 Plus）

> 注（T8a 后修订）：GPT 默认从 `gpt-5.5` 改为 `gpt-5.4-high`。原因：T7 真实测试发现 PackyAPI 服务端对 `/v1/messages` 端点的 `gpt-5.5` 静默 fallback 到 5.4（response model 字段写 5.4），UI 显示对齐实际后端行为。
...
- 不实现 + 号菜单二级菜单（多模型变体选择）。GPT 默认 `gpt-5.4-high`（T8a 修订前为 `gpt-5.5`），要换变体去设置面板改 model 字段（Q1 决策：A）
...
pty.spawn('powershell.exe', { env: sessionEnv, cwd })
   ↓ 写入 stdin：
     " claude --model gpt-5.4-high --permission-mode bypassPermissions\r\n"
   ↓
...
- `model`：暴露给用户编辑。默认 `gpt-5.4-high` / `kimi-k2.5` / `qwen3.6-plus`（T8a 修订前 GPT 默认为 `gpt-5.5`，因 PackyAPI Anthropic 端点静默 fallback 到 5.4 改为对齐）。GPT 用户可改成 `gpt-5.5` / `gpt-5.3-codex-xhigh` 等任意 cxtocc 分组允许的变体（pricing JSON 里 `enable_groups` 含 `cxtocc` 且 `supported_endpoint_types` 含 `anthropic` 的模型均可）
...
- 启动隔离 Hub 实例（`CLAUDE_HUB_DATA_DIR=C:/temp/hub-packy-test`），通过 IPC 触发 `createSession({kind: 'gpt'})`，检查：
  ...
  - 写入 stdin 的命令含 `--model gpt-5.4-high`
- 三类 kind 各跑一遍
