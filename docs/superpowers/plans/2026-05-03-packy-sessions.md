# PackyAPI 多模型 Session 实施计划（GPT 5.4 high / Kimi K2.5 / Qwen 3.6 Plus）

> 注（T8a 后修订）：GPT 默认从 `gpt-5.5` 改为 `gpt-5.4-high`。原因：T7 真实测试发现 PackyAPI 服务端对 `/v1/messages` 端点的 `gpt-5.5` 静默 fallback 到 5.4。Codex CLI（走 OpenAI 端点）的 `codex_api_model='gpt-5.5'` 不变。
...
期望输出：3 行，url 都是 `https://www.packyapi.com`，model 各自默认（gpt-5.4-high/kimi-k2.5/qwen3.6-plus），hasKey 为 false（还没填 key）。
...
```js
      ...
      // 让 Claude Code CLI 连接 PackyAPI 的 Anthropic 兼容端点（gpt-5.4-high 等通过协议翻译）
...
```js
      ...
      const cv = getConfigValues();
      const model = cv.GPT_MODEL || 'gpt-5.4-high';
...
```html
<button class="new-session-option" data-kind="gpt"><span class="ai-logo logo-gpt"></span>GPT 5.4 high</button>
...
```html
<button class="resume-option" data-kind="gpt-resume"><span class="ai-logo logo-gpt"></span>GPT 5.4 high<span class="hint">PTY picker</span></button>
...
<button class="config-ai-row" type="button" data-ai="gpt">
  ...
  <span class="config-ai-copy">
    <strong>GPT</strong>
    <small id="cfg-summary-gpt">API · gpt-5.4-high · Packy</small>
  </span>
...
<div class="config-ai-detail" id="cfg-detail-gpt">
  <div class="config-detail-header">
    ...
    <span>GPT 5.4 high 通过 PackyAPI 的 Anthropic 兼容端点接入（服务端做 OpenAI ↔ Anthropic 协议翻译），使用隔离 Claude 配置目录。</span>
  </div>
  ...
  <div class="config-field">
    <label>模型</label>
    <input type="text" id="cfg-gpt-model" placeholder="gpt-5.4-high" spellcheck="false">
  </div>
</div>
```
