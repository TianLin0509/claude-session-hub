...
> 2026-08-28 更新：新增与正式账本/Session 完全隔离的虚拟实盘调试台，周末手动盘前决策自动定位下一官方交易日，并修正同日开盘 mark 导致收盘日收益分母错误的问题。虚拟模式、启动恢复与验收口径见 `docs/agent-league-p0-daily-hook.md`。
>
> 2026-08-27 更新：日频 P0 已改为“盘前 DRAFT → 同 Session Hook → 当日开盘执行 → 收盘记账；周六沉淀”，账户改为 50 万、沪深全市场（不含北交所）及用户指定费用。下文 2026-08-26 的 T+1 收盘口径保留作历史记录，当前权威说明见 `docs/agent-league-p0-daily-hook.md`。
