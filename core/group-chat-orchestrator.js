...
class GroupChatOrchestrator {
  ...
  buildFirstDelta(selfSid, userInput, systemPromptText, opts = {}) {
    if (this.state.lastDeliveredIdx[selfSid] === undefined) {
      ...
      const cutoff = currentUserMessageAppended
        ? Math.max(0, this.state.messages.length - 1)
        : this.state.messages.length;
      const currentTurnNum = Number(opts.currentTurnNum);
      let inferredCurrentUserIndex = -1;
      if (!currentUserMessageAppended && !Number.isInteger(currentTurnNum)) {
        for (let i = cutoff - 1; i >= 0; i--) {
          const message = this.state.messages[i];
          if (message && message.role === 'user' && String(message.content || '') === String(userInput || '')) {
            inferredCurrentUserIndex = i;
            break;
          }
        }
      }
      const unseenHistory = this.state.messages
        .slice(0, cutoff)
        .filter((_m, index) => index !== inferredCurrentUserIndex)
        .filter(m => m && m.content && m.sid !== selfSid)
        .filter(m => !(m.role === 'user' && Number.isInteger(currentTurnNum) && m.turnNum === currentTurnNum));
      const parts = [String(systemPromptText || '')];
      if (unseenHistory.length > 0) {
        parts.push(
          '## 群聊历史（新成员首次同步）\n'
          + unseenHistory.map(m => `${m.speaker || (m.role === 'user' ? '你' : 'AI')}：${m.content}`).join('\n\n'),
        );
      }
      parts.push('## 用户\n' + (userInput || ''), '请发言。');
      return parts.join('\n\n');
  ...
  completeTurn(turnNum, userInput, results, memberBySid, statsBySid = {}, opts = {}) {
    ...
    const startedAtBy = isExistingTurn && turn.startedAtBy && typeof turn.startedAtBy === 'object' ? turn.startedAtBy : {};
    const completedAtBy = isExistingTurn && turn.completedAtBy && typeof turn.completedAtBy === 'object' ? turn.completedAtBy : {};
    const aiMessages = [];
    const orderedResults = (Array.isArray(results) ? results : [])
      .map((result, index) => ({ result, index }))
      .sort((a, b) => {
        const aCompleted = Number(a.result && a.result.completedAt);
        const bCompleted = Number(b.result && b.result.completedAt);
        const aKey = Number.isFinite(aCompleted) ? aCompleted : Number.MAX_SAFE_INTEGER;
        const bKey = Number.isFinite(bCompleted) ? bCompleted : Number.MAX_SAFE_INTEGER;
    ...
    for (const r of orderedResults) {
      ...
      const resultStartedAt = Number(r.startedAt);
      const resultCompletedAt = Number(r.completedAt);
      if (Number.isFinite(resultStartedAt)) startedAtBy[sid] = resultStartedAt;
      if (Number.isFinite(resultCompletedAt)) completedAtBy[sid] = resultCompletedAt;
      ...
      if (msg) {
        ...
        if (Number.isFinite(resultStartedAt)) msg.startedAt = resultStartedAt;
        if (Number.isFinite(resultCompletedAt)) {
          msg.completedAt = resultCompletedAt;
          msg.createdAt = resultCompletedAt;
        }
        ...
        msg = this._appendMessage({
          ...
          ...(Number.isInteger(Number(opts.workflowStepIndex)) ? { workflowStepIndex: Number(opts.workflowStepIndex) } : {}),
          ...(Number.isFinite(resultStartedAt) ? { startedAt: resultStartedAt } : {}),
          ...(Number.isFinite(resultCompletedAt) ? { completedAt: resultCompletedAt, createdAt: resultCompletedAt } : {}),
    ...
    if (!turn) {
      turn = {
        ...
        meta: {
          ...
          ...(Number.isInteger(Number(opts.workflowStepIndex)) ? { lastWorkflowStepIndex: Number(opts.workflowStepIndex) } : {}),
        },
      };
      ...
      turn.startedAtBy = startedAtBy;
      turn.completedAtBy = completedAtBy;
      ...
      if (Number.isInteger(Number(opts.workflowStepIndex))) turn.meta.lastWorkflowStepIndex = Number(opts.workflowStepIndex);
    }
    this.state.currentMode = 'idle';
    delete this._activePrompts[turnNum];
    const lastIdx = this.state.messages.length - 1;
