...
serve('ai-hub-web-roundtable',[
  ...
  {name:'roundtable_resume',description:'Resume an interrupted coordinator. Deduplicates child calls using their original request IDs; never resends uncertain website submissions.',inputSchema:taskSchema},
  ...
  if(name==='roundtable_cancel'){store.cancel(job.id);return {task_id:job.id,cancelRequested:true};}
});
