...
async function collect(id,provider){return store.locked('create-'+id,async()=>{const job=status(id);if(job.kind!=='web'||job.input.provider!==provider)throw Error('Provider task mismatch');if(job.state==='succeeded')return job;if(!terminal.has(job.state))return job;if(!job.submissionAttempted||!adapters.validUrl(provider,job.url))throw Error('No known submitted conversation to collect; inspect the official website before creating another task');await spawnWorker(id,'collect');return {...job,state:'collecting'};});}
async function resumeWeb(id,provider){const job=status(id);if(job.submissionAttempted)return collect(id,provider);return store.locked('create-'+id,async()=>{const current=status(id);if(current.kind!=='web'||current.input.provider!==provider)throw Error('Provider task mismatch');if(!terminal.has(current.state))return current;if(store.cancelled(id))throw Error('Cancelled tasks are not resumed; create a new request only after inspecting the original conversation');await spawnWorker(id);return {...current,state:'resuming'};});}
async function runWeb(job,save,mode,runtime={}){
  ...
  let release,browser;
  const checkCancel=()=>{if(store.cancelled(job.id))throw Object.assign(Error('Cancelled; a submitted website response may continue remotely'),{cancelled:true});};
  try{
    ...
    do{
      ...
      if(loginCount>=4||snap.challenge)throw Object.assign(Error('Official website requires login or human verification after submission; not resending'),{attention:true});
  ...
  }catch(e){save({state:e.cancelled?'cancelled':job.submissionAttempted||e.attention?'needs_attention':'failed',error:e.message,...(browser?.page.networkErrors?.length?{networkErrors:browser.page.networkErrors}:{})});}
  finally{if(browser)try{await browser.close();}catch(e){save({cleanupError:e.message});}if(release)release();}
}
module.exports={terminal,text,status,create,ask,collect,resumeWeb,spawnWorker,runWeb};
