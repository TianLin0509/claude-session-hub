...
class HubMemoryService {
  ...
  async context(sessionId) {
    ...
    const nativeTask = this.nativeContextReader.read({ ...s }).catch(error => ({ entries: [], warnings: ['原生注入记录读取失败：' + error.message] }));
    let history;
    ...
    if (identity !== contextIdentity(this.session(sessionId))) throw new Error('当前原生会话已切换，请刷新重试');
    return {
