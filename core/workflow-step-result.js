...
/**
 * 这一步的活状态盘点。
 *   targetSids  这一步该交货的全部席位
 * 返回里 missingSids 是「还缺谁」——重发只该针对它们，已经答过的绝不再问一遍。
 */
function inspectStepResults({ turn, messages, turnNum, targetSids } = {}) {
  const sids = (Array.isArray(targetSids) ? targetSids : []).filter(Boolean);
  const results = sids.map(sid => liveResultFor({ turn, messages, turnNum, sid }));
  const missingSids = results.filter(r => !resultIsUsable(r)).map(r => r.sid);
  return {
    total: sids.length,
    results,
    missingSids,
 ...
 */
function decideResumeAction({ turn, messages, turnNum, targetSids, stopped = false, discussing = false } = {}) {
  ...
  const view = inspectStepResults({ turn, messages, turnNum, targetSids });
