...
class ClaudeNativeActivities {
  ...
  abandonStale() {
    ...
    // 段内的帧就是它自己的输出。判 unknown 之前先落回它名下，否则紧接着的
    // deferSegment 会把已经收下的正文撤销，只剩一条空壳记录。
    if (this.current && stale.includes(this.current) && this.segment.length) this.resolveSegment(this.current);
