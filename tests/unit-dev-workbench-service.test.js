...
async function main(){
  ...
  s=setup([make('placement')],{loopEngine:null});
  try{
    check((await s.act('placement','pin')).ok&&s.row('placement').pinned,'Pin works even when the execution engine is unavailable');
    check((await s.act('placement','bottom')).ok&&s.row('placement').bottomed&&!s.row('placement').pinned,'Bottom placement clears pin in shared meeting metadata');
    check((await s.act('placement','unbottom')).ok&&!s.row('placement').bottomed,'Removing placement restores chronological order');
    s.board.ingest({hubDataDir:directory,meetingId:'placement',summary:Feed.summarizeGroupState({revision:12,messages:[{role:'assistant',content:'PROGRESS: 原实现',createdAt:10},{role:'assistant',content:'RESULT: FAIL\nBLOCKERS: 新审核意见',createdAt:20}]})});
    check(s.row('placement').progress.includes('新审核意见'),'Latest review appears as latest progress instead of an older author report');
    s.board.ingest({hubDataDir:directory,meetingId:'placement',summary:Feed.summarizeGroupState({revision:11,messages:[{role:'assistant',content:'UPDATE: 迟到旧状态',createdAt:30}]})});
    check(s.row('placement').progress.includes('新审核意见'),'Foundation revision rejects late stale projections despite later wall-clock timestamps');
    check(s.row('placement').activityAt===20,'Sorting uses durable activity time rather than reload/arrival time');
  }finally{s.board.dispose();}
  s=setup([make('active','done')]);
  try{
    s.board.ingest({hubDataDir:directory,meetingId:'active',summary:{execution:{runId:'new-run',status:'running',attempts:[{status:'running',memberId:'m8'}]}}});
    check(s.row('active').stage.key==='chatting','New active chat attempt supersedes an old completed loop status');
    s.board.ingest({hubDataDir:directory,meetingId:'active',summary:{execution:{runId:'new-run',status:'recovering',attempts:[{status:'recovering',memberId:'m8'}]}}});
    check(s.row('active').stage.key==='recovering','Restart recovery is surfaced as a task that needs attention');
    s.board.ingest({hubDataDir:directory,meetingId:'active',summary:{execution:{runId:'new-run',hasFailures:true,attempts:[{status:'failed',failure:{summary:'本轮执行失败'}}]}}});
    check(s.row('active').stage.key==='chatFailed'&&s.row('active').lastError==='本轮执行失败','A new failed attempt cannot inherit an old passed badge');
  }finally{s.board.dispose();}
  s=setup([make('cold-active')],{readSummary:async()=>({summary:{execution:{runId:'disk-run',attempts:[{attemptId:'disk-attempt',status:'running'}]}}})});
  try{
    s.board.snapshot();await wait(10);
    check(s.row('cold-active').stage.key==='recovering','Disk-only running attempts are awaiting recovery rather than falsely live');
    s.board.ingest({hubDataDir:directory,meetingId:'cold-active',summary:{execution:{runId:'disk-run',attempts:[{attemptId:'disk-attempt',status:'running'}]}}});
    check(s.row('cold-active').stage.key==='chatting'&&!s.row('cold-active').actions.resume,'Live chat blocks duplicate workflow resumption');
  }finally{s.board.dispose();}
