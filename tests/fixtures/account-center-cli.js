...
let result;
if(action==='images')result={ok:true,accounts:['primary','secondary'].flatMap(loginGroup=>Array.from({length:4},(_,i)=>({id:loginGroup+(i?'-'+(i+1):''),loginGroup,enabled:true,state:loginGroup==='secondary'&&i===2?'login_required':'signed_in',workerAlive:false,observedAt:Date.now()})))};
