...
if(action==='images')result={ok:true,accounts:['primary','secondary'].flatMap(loginGroup=>Array.from({length:4},(_,i)=>({id:loginGroup+(i?'-'+(i+1):''),loginGroup,enabled:true,state:loginGroup==='secondary'&&i===2?'login_required':'signed_in',workerAlive:false,observedAt:fs.statSync(trace).birthtimeMs,account_name:loginGroup==='primary'?'FIXTURE POOL':''})))};
...
else result={ok:true,accountLabel:row.id==='bridge'?'fixture-bridge@example.com':/^codex-|^claude$/.test(row.id||'')?'fixture-main@example.com':'',state:row.id==='claude'||row.id==='codex-default'||/^image-primary(?:-[234])?$/.test(row.id)||fs.existsSync(path.join(root,'fixture-login-'+row.id))?'signed_in':'unknown',identity:'fixture@example.com',source:'原生协议夹具，不是真实账号',message:'测试状态',observedAt:Date.now()};
