...
t('v2 四行契约 PASS', () => {
  const v = LW.parseVerdict('审查完成\nRESULT: PASS\nBLOCKERS: 无\nVERIFIED: npm test 42/42\nNEXT: 无');
  assert(v && v.decision === 'pass' && v.blockers.length === 0 && v.verified.length === 1);
});
t('v2 四行契约 FAIL + 中文冒号', () => {
  const v = LW.parseVerdict('RESULT：FAIL\nBLOCKERS：登录仍返回 500\nVERIFIED：curl /login 复现\nNEXT：补回归测试');
  assert(v && v.decision === 'fail' && /登录/.test(v.blockers[0].what));
  assert(v.suggestions.length === 1 && /回归/.test(v.suggestions[0].idea));
});
...
t('FAIL 未给 blockers → 生成可回灌占位阻断项', () => {
  const r = LW.mergeVerdicts([{ from: 'Claude', verdict: { decision: 'fail', blockers: [], verified: ['x'] }, raw: '' }]);
  assert(r.pass === false && r.blockers.some(b => /未给出阻断项/.test(b.what)));
});
...
t('reaching+pass(有 polish+池非空) → polishing', () => {
  const s = LW.newLoopState();
  LW.advanceLoopState(s, { pass: true, blockers: [], suggestions: [{ idea: 'x' }], fullVerdicts: [] }, cfg({ gate: { consecutivePass: 1 }, polish: { enabled: true } }), 0);
...
t('polishing 不再吸收评审新建议，建议池可收敛', () => {
  const s = LW.newLoopState();
  s.phase = 'polishing'; s.suggestionPool = [{ idea: '旧建议' }];
  LW.advanceLoopState(s, { pass: true, blockers: [], suggestions: [{ idea: '本轮新建议不入池' }], fullVerdicts: [] }, cfg({ polish: { enabled: true } }), 0);
  assert(s.status === 'done' && s.suggestionPool.length === 0, 'status=' + s.status + ' pool=' + s.suggestionPool.length);
});
t('强制退出①：maxRounds', () => {
