...
test('ancestor rules are not dropped merely because a native file exists or a repository boundary is present',t=>{
 ...
 write(path.join(cwd,'AGENTS.md'),'workspace rule');assert.equal(sharedWorkspaceRules(request)[0].content,'workspace rule');
 fs.mkdirSync(path.join(root,'.git'));assert.equal(sharedWorkspaceRules(request)[0].content,'workspace rule');
 write(path.join(home,'.codex','AGENTS.md'),'unrelated global rule'.repeat(5000));
 assert.equal(sharedWorkspaceRules(request)[0].content,'workspace rule','unrelated large native rules must not block submission');
...
test('Codex rules and developer memory use the same validated reader and leave historical data unchanged',async t=>{
 ...
 write(file,raw);
 const reader=new NativeContextReader(),s={kind:'codex',codexSid:'n1',transcriptPath:file};
 const result=await reader.read(s);assert.equal(result.entries.length,2);assert.match(result.entries[0].content,/历史有效经验/);assert.match(result.entries[1].content,/历史提炼经验/);
 assert.equal(result.entries[0].path,file);assert.equal(fs.readFileSync(file,'utf8'),raw);
 await assert.rejects(reader.read({...s,codexSid:'n2'}),/身份/);
});
