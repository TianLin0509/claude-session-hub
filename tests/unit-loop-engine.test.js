...
function mk(opts) {
  opts = opts || {};
  const baseWf = { steps: [['m1'], ['m2']], loop: Object.assign({ enabled: true, maxRounds: 2, consecutivePass: 1, polish: false }, opts.loop || {}) };
  ...
  const deps = {
    getDispatcher: () => ({
      dispatchGroupChatTurn: async (mid, args) => {
        turnCalls.push(args);
        if (opts.dispatchResult) return opts.dispatchResult(turnCalls, args);
...
async function main() {
  ...
  await t('开发步被新提问抢占后立即停止，不再派发评审步', async () => {
    const m = mk({
      dispatchResult: () => ({ status: 'completed', superseded: true, turnNum: 1, results: [] }),
    });
    const eng = createLoopEngine(m.deps);
    const st = await eng.runLoop('mtg', 'g', null, { clientGeneration: 7 });
    assert(st && st.status === 'stopped_superseded', 'status=' + (st && st.status));
    assert.strictEqual(m.turnCalls.length, 1, '被抢占后不应继续派发评审步');
    assert.strictEqual(m.turnCalls[0].clientGeneration, 7, '循环内派发应透传 renderer generation');
  });
