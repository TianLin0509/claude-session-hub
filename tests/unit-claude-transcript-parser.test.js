...
test('regression: skips task-notification system user entries from background commands', () => {
  ...
  const taskNotification = JSON.stringify({
    type: 'user',
    uuid: 'u-task',
    timestamp: '2026-06-14T13:47:12.723Z',
    message: {
      role: 'user',
      content: '<task-notification>\n<task-id>b1bwbajl9</task-id>\n<tool-use-id>toolu_01X7</tool-use-id>\n<output-file>C:\\Users\\lintian\\AppData\\Local\\Temp\\claude\\tasks\\b1bwbajl9.output</output-file>\n<status>completed</status>\n<summary>Background command "Verify videos" completed (exit code 0)</summary>\n</task-notification>',
    },
  ...
  const assistant = JSON.stringify({
    type: 'assistant',
    uuid: 'a-1',
    timestamp: '2026-06-14T13:48:00.000Z',
    message: {
  ...
  const fp = writeTmp('skip-task-notification.jsonl', [realUser, taskNotification, assistant].join('\n') + '\n');
  try {
    const turns = parseClaudeTranscriptToTurns(fp);
    assert.deepStrictEqual(turns.map(t => t.id), ['u-real', 'a-1']);
    assert.ok(!turns.some(t => String(t.text || '').includes('b1bwbajl9')));
  } finally {
    cleanup(fp);
  }
});

test('regression: skips compact/local-command metadata user entries', () => {
  const compactSummary = JSON.stringify({
    type: 'user',
    uuid: 'u-compact',
    timestamp: '2026-06-15T14:47:50.967Z',
  ...
  const caveat = JSON.stringify({
    type: 'user',
    uuid: 'u-caveat',
    timestamp: '2026-06-15T14:45:49.563Z',
    isMeta: true,
    message: {
      role: 'user',
  ...
  const realUser = JSON.stringify({
    type: 'user',
    uuid: 'u-real',
    timestamp: '2026-06-15T14:50:00.000Z',
    message: { role: 'user', content: 'fix it' },
  });
  const fp = writeTmp('skip-metadata-users.jsonl', [compactSummary, caveat, realUser].join('\n') + '\n');
  try {
    const turns = parseClaudeTranscriptToTurns(fp);
    assert.strictEqual(turns.length, 1);
    assert.strictEqual(turns[0].id, 'u-real');
    assert.strictEqual(turns[0].text, 'fix it');
  } finally {
    cleanup(fp);
  }
});
