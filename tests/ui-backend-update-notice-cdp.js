...
(async () => {
  ...
  try {
    ...
    if (baseline) {
      ...
      await click('.backend-update-toggle');
      const narrow = await c.eval(`(()=>{const r=document.querySelector('#backend-update-details').getBoundingClientRect(),p=document.querySelector('#terminal-panel').getBoundingClientRect();return{left:r.left,right:r.right,top:r.top,bottom:r.bottom,panelLeft:p.left,panelRight:p.right,width:innerWidth,pageWidth:document.documentElement.scrollWidth};})()`);
      assert(narrow.left >= narrow.panelLeft && narrow.right <= narrow.panelRight && narrow.top >= 0 && narrow.bottom < 844 && narrow.pageWidth === 390, j(narrow));
