...
async function main(){
  ...
  try{
    ...
    const choose=async(selector,value)=>{await ev(`(()=>{const el=document.querySelector(${JSON.stringify(selector)});el.value=${JSON.stringify(value)};el.dispatchEvent(new Event('change',{bubbles:true}));})()`);await _waitMs(120);};
    const screenshot=async(name)=>{const image=await client.send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(OUT,`20260905-dev-workbench-rows-${name}-codex1.png`),Buffer.from(image.data,'base64'));};
    await waitFor(`typeof window.__devBoardShow==='function'&&!!document.querySelector('#devb-status')`,'module and event handlers ready');
    await ev('document.fonts.ready.then(()=>true)');await _waitMs(600);
    ...
    const menu=async id=>{if(!await ev(`document.querySelector(${JSON.stringify(row(id)+' .devb-menu')}).open`))await click(row(id)+' .devb-menu summary');};
    report.metrics.rowGeometry=await ev(`Array.from(document.querySelectorAll('.devb-row'),el=>{const r=el.getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,bottom:r.bottom}})`);
    // Windows fractional zoom produces adjacent bounds differing by ~0.00003px.
    check(report.metrics.rowGeometry.every((r,i,rows)=>Math.abs(r.x-rows[0].x)<1&&Math.abs(r.width-rows[0].width)<1&&(!i||r.y>=rows[i-1].bottom-.01)),'Tasks occupy distinct full-width horizontal rows');
    check(await ev(`document.querySelector(${JSON.stringify(row(ids[0])+' .devb-flow [aria-current="step"]')}).textContent.includes('审核')`),'Paused task retains its current position in the complete visible workflow');
    await ev(`document.documentElement.dataset.theme='claude';true`);
    check(await ev(`getComputedStyle(document.querySelector('#ran-panel')).backgroundColor==='rgb(255, 255, 255)'`),'Quiet A has the selected white surface in the light Hub theme');
    check(await ev(`getComputedStyle(document.querySelector('.devb-row')).borderTopWidth==='0px'&&getComputedStyle(document.querySelector('.devb-row')).borderLeftWidth==='0px'`),'Task rows use simple separators without card borders or colored rails');
    for(const theme of ['dark','claude','codex','hub','slate']){
      await ev(`themeController.setTheme(${JSON.stringify(theme)});true`);
      const ratio=await ev(`(()=>{const s=getComputedStyle(document.querySelector('#ran-panel .devb-primary'));const luminance=color=>color.match(/[\\d.]+/g).slice(0,3).map(Number).map(v=>{v/=255;return v<=.04045?v/12.92:((v+.055)/1.055)**2.4}).reduce((sum,v,i)=>sum+v*[.2126,.7152,.0722][i],0);const a=luminance(s.color),b=luminance(s.backgroundColor);return (Math.max(a,b)+.05)/(Math.min(a,b)+.05)})()`);
      check(ratio>=4.5,theme+': primary action remains readable after real theme-controller switching');
    }
    await ev(`themeController.setTheme('claude');true`);
    await screenshot('quiet-overview-light');
    await choose('#devb-sort','updated-desc');
    await click(row(ids[0])+' [data-devb-action="details"]');
    const readingOrder=await ev(`Array.from(document.querySelectorAll('.devb-row'),el=>el.dataset.mid).join(',')`);
    await ev(`require('electron').ipcRenderer.invoke('fixture:dev-publish',{id:${JSON.stringify(ids[0])},text:'检查 A 风格中的实时进展与阅读位置',progress:true})`);
    await waitFor(`document.querySelector(${JSON.stringify(row(ids[0]))}).textContent.includes('检查 A 风格中的实时进展与阅读位置')`,'A live reading update');
    check(await ev(`Array.from(document.querySelectorAll('.devb-row'),el=>el.dataset.mid).join(',')`)===readingOrder,'Real published progress updates content without reordering the task being read');
    check(await ev(`!document.querySelector('#devb-updates').hidden`),'New progress offers an explicit chronological reorder control');
    ...
    check(await ev(`document.querySelector('.devb-row-main').getBoundingClientRect().height`)<comfortableHeight,'Compact density reduces row height');
    await screenshot('quiet-compact-light');
    await ev(`document.documentElement.dataset.theme='dark';true`);
    await choose('#devb-sort','created-desc');
    check(await ev(`document.querySelector('.devb-row').dataset.mid===${JSON.stringify(ids[0])}`),'Creation time order is independent of the latest notification arrival');
    await menu(ids[1]);await click(row(ids[1])+' [data-devb-action="pin"]');
    await waitFor(`document.querySelector('.devb-row').dataset.mid===${JSON.stringify(ids[1])}`,'pin moves task first');
    check(await ev(`document.querySelector(${JSON.stringify(row(ids[1])+' .devb-placement')}).textContent==='置顶'`),'Pin moves a task to the top through real meeting IPC');
    await menu(ids[1]);await click(row(ids[1])+' [data-devb-action="bottom"]');
    await waitFor(`[...document.querySelectorAll('.devb-row')].at(-1).dataset.mid===${JSON.stringify(ids[1])}`,'bottom moves last');
    check(await ev(`!document.querySelector('#devb-confirm').open`),'Reversible placement changes need no workflow confirmation');
    await click('#devb-refresh');await _waitMs(150);
    check(await ev(`document.querySelector(${JSON.stringify(row(ids[1])+' .devb-placement')}).textContent==='置底'`),'Placement survives snapshot reload');
    check(await ev(`(async()=>{const saved=await require('electron').ipcRenderer.invoke('fixture:dev-saved-placement',{id:${JSON.stringify(ids[1])}});return saved.bottomed===true&&saved.pinned===false})()`),'Bottom placement is persisted in the authoritative meeting file');
    await menu(ids[1]);await click(row(ids[1])+' [data-devb-action="unbottom"]');await _waitMs(160);
    await click('[data-mode="projects"]');
    check(await ev(`document.querySelectorAll('.devb-project-row').length===3`),'Project view separates same-name projects with different workspaces');
    check(await ev(`!document.querySelector(${JSON.stringify(row(ids[2]))})&&!!document.querySelector(${JSON.stringify(row(ids[0]))})`),'Project row shows current tasks while passed history is initially collapsed');
    const historySelector=await ev(`(()=>{const p=document.querySelector(${JSON.stringify(row(ids[0]))}).closest('.devb-project-row');return '[data-project-key="'+p.dataset.projectKey+'"] [data-devb-action="history"]'})()`);
    await click(historySelector);
    check(await ev(`!!document.querySelector(${JSON.stringify(row(ids[2]))})`),'Expanding history reveals the passed task within its own project');
    await screenshot('projects-dark');
    await ev(`document.documentElement.dataset.theme='claude';true`);await screenshot('projects-light');
    await ev(`document.documentElement.dataset.theme='dark';true`);
    await click('[data-mode="tasks"]');
    await menu(ids[0]);
    ...
    await menu(ids[0]);await click(row(ids[0])+' [data-devb-action="restore"]');await click('[data-devb-action="confirm"]');
    ...
    await click(row(ids[1])+' [data-devb-action="details"]');
    ...
    check(await ev(`document.querySelectorAll('.devb-row').length===40`),'1,000 tasks render at most 40 rows per page');
    ...
    await click('[data-mode="projects"]');
    check(await ev(`document.querySelectorAll('.devb-project-row').length===3&&document.querySelectorAll('.devb-row').length<=17`),'Project mode bounds initial task rendering under a 1,000-task load');
    const projectBefore=await ev(`document.querySelectorAll('.devb-project-current .devb-row').length`);
    await click('[data-devb-action="more-current"]');
    check(await ev(`document.querySelectorAll('.devb-project-current .devb-row').length===${projectBefore+10}`),'More-current control reveals additional tasks without losing the project grouping');
    await ev(`require('electron').ipcRenderer.invoke('fixture:dev-publish',{id:${JSON.stringify(ids[0])},text:'项目模式实时进展',progress:true})`);
    await waitFor(`document.querySelector(${JSON.stringify(row(ids[0]))}).textContent.includes('项目模式实时进展')`,'project push');
    check(await ev(`document.querySelector('[data-mode="projects"]').getAttribute('aria-selected')==='true'&&document.querySelectorAll('.devb-project-current .devb-row').length===${projectBefore+10}`),'Project updates preserve the selected tab and expanded current-task count');
    check(await ev(`document.querySelector(${JSON.stringify(row(ids[2]))})?.closest('.devb-project-history')!=null`),'Expanded project history remains open across live progress updates');
    await click('[data-mode="tasks"]');
    ...
    check(await ev(`!document.querySelector(${JSON.stringify(row(ids[0])+' .devb-details')}).hidden`),'Pushed updates preserve expanded task details');
    ...
    check(await ev(`document.querySelector('#devb-sort').value==='created-desc'`),'Chosen chronological sort survives renderer restart');
    check(await ev(`document.querySelector('#ran-panel').classList.contains('devb-compact')&&document.querySelector('#devb-density').getAttribute('aria-pressed')==='true'`),'Density preference survives renderer restart');
    await click('[data-mode="projects"]');await ev(`location.reload();true`);await waitFor(`typeof window.__devBoardShow==='function'`,'project renderer reload');await click('#btn-ran');
    await waitFor(`document.querySelectorAll('.devb-project-row').length===3`,'saved project view');
    check(await ev(`document.querySelector('[data-mode="projects"]').getAttribute('aria-selected')==='true'`),'Project tab preference survives renderer restart');
    ...
    if(hub){report.shutdown=await gracefulQuit(hub);report.exitCode=hub.exitCode();fs.writeFileSync(path.join(OUT,'20260905-dev-workbench-rows-hub-codex1.log'),hub.log().join('\n'),'utf8');}
    fs.writeFileSync(path.join(OUT,'20260905-dev-workbench-rows-e2e-codex1.json'),JSON.stringify(report,null,2),'utf8');
  }
