...
async function main(){
  ...
  try{
    ...
    check(await ev(`(()=>{const rows=[...document.querySelectorAll('.devb-row')].map(el=>el.getBoundingClientRect());return rows.every(r=>Math.abs(r.x-rows[0].x)<1&&Math.abs(r.width-rows[0].width)<1)&&rows.every((r,i)=>!i||r.y>=rows[i-1].bottom)})()`),'Tasks occupy distinct full-width horizontal rows');
