...
async function run(){
  ...
  try{
    ...
    ok('重复补收不造重复卡', (await invoke('groupchat:get-state',{meetingId:id})).messages.filter(m=>m.sourceMessage).length===46);
    ...
    await stop();await start();await cdp.eval(`selectMeeting(${JSON.stringify(id)})`);
