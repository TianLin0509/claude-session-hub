...
test('leaderboard keeps eight compact rows and opens real Session card or PTY views', () => {
  ...
  assert.match(ui, /agent-league:execute-open/);
  assert.match(ui, /agent-league:record-close/);
  assert.match(ui, /agent-league:run-weekly/);
  ...
  assert.match(ui, /agent-league:prompt-files/);
  assert.match(ui, /agent-league:save-prompt-file/);
  assert.match(ui, /受保护的机器状态/);
  assert.match(ui, /待首次运行/);
  assert.match(ui, /agent-league:ensure-session[\s\S]*bridge\.open\(ensured\.session\.id, view, ensured\.session\)/);
  assert.match(ui, /盘前决策已启动，正在打开对应 Agent PTY/);
