...
async function main(){
  ...
  try{
    hub=await launchIsolatedHub({dataDir:path.join(root,'data'),port:await port(),label:'other-providers',extraEnv:{CODEX_HOME:path.join(root,'codex'),CLAUDE_CONFIG_DIR:path.join(root,'claude'),KIMI_CODE_HOME:path.join(root,'kimi'),KIMI_CODE_BIN:path.join(os.homedir(),'.kimi-code/bin/kimi.exe')}});cdp=await connectFirstPage(hub);await until('typeof sessions!=="undefined"','renderer');
    for(const kind of ['claude','kimi']){
      ...
      await until('document.querySelector(".floating-input-box")','legacy composer');assert.equal(await cdp.eval('!!document.querySelector(".codex-native-controls:not([hidden])")'),false);
      await sleep(6000);
