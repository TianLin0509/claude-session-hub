...
// slot 化（2026-05-03）：圆桌 sid 标签现以 slot 名（皮卡丘/小火龙/杰尼龟）出现，
//   不再以底层 AI 家族名（Claude/Gemini/Codex）出现 — main.js sidLabelFn 改用 slotIndexToId+getSlotPromptName。
// 称谓中文化（2026-05-08）：getSlotPromptName 返回中文，labelMap 同步反映线上实际值。
const labelMap = { sid_a: '皮卡丘', sid_b: '小火龙', sid_c: '杰尼龟' };
...
function testBriefSummaryPromptOutput() {
  ...
  // 标题
  assert.ok(prompt.includes('第 5 轮 · 摘要轮 · by 皮卡丘'), 'title with summarizer label');
  // 任务说明
...
function testTimelineWritesSummaryTurnHeader() {
  ...
  assert.ok(content.includes('## 第 2 轮 · 摘要 by 皮卡丘（五元组）'),
