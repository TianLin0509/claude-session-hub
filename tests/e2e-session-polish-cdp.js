...
async function main() {
  ...
  try {
    ...
    assert.equal(await client.eval('document.querySelector(".view-toggle-btn.active").dataset.view'),'card');
    ...
    await clickPoint(client,'#btn-home');
    await clickPoint(client,'.session-item[data-session-id="'+created.id+'"]');
    assert.equal(await client.eval('document.querySelector(".view-toggle-btn.active").dataset.view'),'pty');
