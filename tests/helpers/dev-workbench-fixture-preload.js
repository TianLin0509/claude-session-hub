...
if (process.type === 'browser' && process.env.HUB_DEV_WORKBENCH_FIXTURE === '1') {
  ...
  ipcMain.handle('fixture:dev-seed',(_event,{count=6}={})=>{
    for(let i=ids.length;i<count;i++){
      ...
      manager.meetings.get(m.id).createdAt=Date.now()-86400000*(i+1);
  ...
  ipcMain.handle('fixture:dev-saved-placement',async(_event,{id}={})=>{
    if(!ids.includes(id))throw new Error('unknown fixture task');
    const store=require('../../core/meeting-store');await store.flushAll();
    const saved=store.loadMeetingFile(id);return {pinned:saved?.pinned,bottomed:saved?.bottomed};
  });
}
