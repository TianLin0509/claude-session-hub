...
async function main(){
  ...
  try{
    ...
    const selected=originalEffort==='max'?'xhigh':'max';
    for(const effort of [selected,originalEffort]){
      const before=s.history.size,count=fixture.requests.length;
      const choice=await s.configure({model,effort});assert.equal(choice.appliesOn,'next-turn');assert.equal(s.history.size,before);assert.equal(fixture.requests.length,count);
      ...
      const request=fixture.requests.filter(r=>r.purpose==='workload').at(-1);assert.equal(request.model,model);assert.equal(request.effort,effort);assert.equal(request.serviceTier,tier==='fast'?'priority':tier);
      result.checks.push({effort,turnId:s.runtime.turnId,serviceTier:request.serviceTier});
    }
    result.passed=true;
  }finally{
    ...
    const out=path.resolve('artifacts/codex-native-runtime/real-settings-'+Date.now()+'.json');fs.writeFileSync(out,JSON.stringify(result,null,2));console.log(JSON.stringify({out,passed:result.passed,checks:result.checks}));
  }
}
