...
//
// 三层 AND：时间门 5s + marker 连续 3 次同 N + 持续 ≥8s + activity 涨 <200 字节。
...
// === 时间门 ===
test('时间门：dispatch 后 < 5s tick 一律 unknown（即使 marker 在）', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 1000);  // 1s 前 dispatch
  const r = detector.tick(sid, '> [[Pasted Content 100 chars]]', 1000);
  assert.strictEqual(r, 'unknown', '5s 内应永远 unknown');
});

// === 没 marker → ok ===
test('5s 后无 marker → ok（streaming 中 / 已提交）', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 6000);
  const r = detector.tick(sid, 'normal streaming output...', 5000);
  assert.strictEqual(r, 'ok');
});

// === marker 第 1 次出现 → unknown，不立即报 ===
test('5s 后 marker 第 1 次看到 → unknown（建立基线，等下一次确认）', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 6000);
...
// === 三层全满足 → stuck ===
test('三层全满足：marker 连续 3 次同 N + 持续 ≥8s + activity 涨 <200 → stuck', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 16000);
  const buf = '> [[Pasted Content 4834 chars]]\n  gpt-5.5 medium · Context 100%';
  // 第 1 次：建基线
  let r = detector.tick(sid, buf, 100);
  assert.strictEqual(r, 'unknown');

  // 通过 _injectEntry 模拟 markerFirstSeenTs 是 9s 前（满足 obsDuration ≥ 8s）
  detector._injectEntry(sid, { markerFirstSeenTs: Date.now() - 9000 });

  // 第 2 次：N 同，计数=2，仍不满足 MIN_MARKER_OBSERVATIONS=3
  r = detector.tick(sid, buf, 150);
  assert.strictEqual(r, 'unknown', '第 2 次仍 unknown（计数=2）');

  // 第 3 次：N 同，计数=3 + obsDuration 9s + activity 涨幅 (150-100)=50 < 200 → stuck
  r = detector.tick(sid, buf, 150);
  assert.strictEqual(r, 'stuck', '第 3 次：三层全满足 stuck');
});

// === 计数够但 streaming 中（activity 涨幅 ≥ 200）→ 不报 stuck ===
test('marker 持续 3 次 + 8s 但 activity 涨幅 ≥200 → 仍 unknown（streaming 慢，非 stuck）', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 16000);
  const buf = '> [[Pasted Content 1000 chars]]';
  detector.tick(sid, buf, 1000);  // 基线 activity=1000
  detector._injectEntry(sid, { markerFirstSeenTs: Date.now() - 9000 });
  detector.tick(sid, buf, 1100);
...
// === 持续时间不够（< 8s） → 不报 stuck ===
test('marker 连续 3 次但持续 < 8s → unknown', () => {
  const sid = 'sid-A';
  detector.start(sid, Date.now() - 16000);
  const buf = '> [[Pasted Content 100 chars]]';
  detector.tick(sid, buf, 100);
  // markerFirstSeenTs 不调（保持现在），后续 tick 间隔 ms 级，obsDuration 不到 8s
  detector.tick(sid, buf, 110);
  const r = detector.tick(sid, buf, 120);
  assert.strictEqual(r, 'unknown', '持续不到 8s 不报 stuck');
});
