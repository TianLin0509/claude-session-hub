...
async function main() {
  ...
  try {
    ...
    // The current workflow waits for the whole stage receipt (settingsVersion
    // 1), even when the delivery file arrives first. It no longer queues the
    // next model submission while the previous stage is still running.
    await wait('build file handoff observed', async () => (await phase()).phase === 'build');
    assert.equal(received().length, 1);
    assert.equal(Object.keys((await state()).attempts).length, 1);
    await shot('build-awaiting-receipt'); checks.push('early kickoff file advances progress but does not send build before the stage receipt');
    ...
    await wait('merge file handoff observed', async () => (await phase()).phase === 'merge');
    ...
    checks.push('the merge member starts after the exact build receipt; old build output cannot settle merge');
