...
// ── 池内票（百合花 603823，最新池 chase 型，门户验证 chase~98.9）──
const bh = screener.brief('603823');
assert.strictEqual(bh.available, true, '603823 应在池内 available=true');
assert.strictEqual(bh.in_pool, true, '603823 in_pool=true');
...
// ── 代码归一化：带 .SH 后缀也命中同一条 ──
const bh2 = screener.brief('603823.SH');
assert.strictEqual(bh2.available, true, '带 .SH 后缀也应命中');
assert.strictEqual(bh2.chase_score, bh.chase_score, '后缀归一化后结果一致');

// ── 池外票（茅台 600519 非强势候选，降级 available=false）──
const mt = screener.brief('600519');
assert.strictEqual(mt.available, false, '600519 池外 available=false');
assert.ok(mt.note && mt.note.includes('不脑补'), '降级 note 必须标注「不脑补」');

// ── promptBlock 文本块 ──
const pbIn = screener.promptBlock('603823');
...
const pbOut = screener.promptBlock('600519');
assert.ok(pbOut.includes('技术初筛') && pbOut.includes('研判'), 'promptBlock 池外降级文本');

// ── mtime 缓存：连续两次结果一致、不崩 ──
const a = screener.brief('603823');
const b = screener.brief('603823');
...
console.log('screener-score ok | 603823:', bh.mode, 'score=' + bh.screener_score, 'name=' + bh.name,
  '| end_date=' + bh.end_date, '| 600519 池外降级=' + (mt.available === false));
