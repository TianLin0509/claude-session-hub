...
async function main() {
  ...
  try {
    ...
    await click(mic);
    await until('document.querySelector(' + JSON.stringify(status) + ').textContent.includes("权限被拒绝")', 'permission failure');
    ...
    await draft(box, '已有草稿：'); await click(mic);
    await until('document.querySelector(' + JSON.stringify(box) + ').textContent.includes("不要合并")', 'partial directly in draft');
    assert.equal(await cdp.eval('document.querySelector(' + JSON.stringify(box) + ').textContent'), '已有草稿：不要合并');
    assert.equal(await cdp.eval('document.querySelectorAll(".voice-panel,.voice-preview").length'), 0);
    const placement = await cdp.eval('(()=>{const a=document.querySelector(".composer-start-actions"),b=document.querySelector(".fi-bridge-toolbar"),i=document.querySelector(".floating-input-box");return {sameRow:a.parentElement===b.parentElement,after:b.nextElementSibling===a,below:a.getBoundingClientRect().top>=i.getBoundingClientRect().bottom};})()');
    assert.deepEqual(placement, { sameRow: true, after: true, below: true });
    ...
    await snap('ordinary-draft'); evidence.checks.push('Live partial in existing draft, final replaces it without duplication; status in existing row; no extra panel; start button after pull/fork below input');
    ...
    await click(mic); await until('document.querySelector(' + JSON.stringify(box) + ').textContent.includes("不要合并")', 'second recording');
    await draft(box, '我同时修改的草稿');
    await until('document.querySelector(' + JSON.stringify(mic) + ').textContent === "语音"', 'editing stops capture');
    await sleep(500);
    assert.equal(await cdp.eval('document.querySelector(' + JSON.stringify(box) + ').textContent'), '我同时修改的草稿');
    evidence.checks.push('Editing stops capture and preserves user changes; no cancel/recovery button required');
    // Replace a selection in the middle, preserving text and attachment nodes on either side.
    await cdp.eval('(()=>{const b=document.querySelector(' + JSON.stringify(box) + ');b.innerHTML="前文<span>待替换</span><b data-test-attachment>附件</b>后文";b.dispatchEvent(new Event("input",{bubbles:true}));b.focus();const r=document.createRange();r.selectNodeContents(b.querySelector("span"));getSelection().removeAllRanges();getSelection().addRange(r);window.__voiceAttachment=b.querySelector("b");})()');
    await click(mic); await until('document.querySelector(' + JSON.stringify(box) + ').textContent.includes("不要合并")', 'middle partial');
    await click(mic); await until('document.querySelector(' + JSON.stringify(box) + ').textContent.includes("SRS")', 'middle final');
    assert.equal(await cdp.eval('document.querySelector(' + JSON.stringify(box) + ').textContent'), '前文不要合并 master。SRS 是十毫秒。附件后文');
    assert.equal(await cdp.eval('window.__voiceAttachment===document.querySelector(' + JSON.stringify(box) + ').querySelector("b")'), true);
    evidence.checks.push('Selected middle text replaced while surrounding text and attachment DOM stay intact');
    mode = 'disconnect'; await click(mic);
    await until('document.querySelector(' + JSON.stringify(status) + ').textContent.includes("连接中断")', 'disconnect visible');
    await snap('network-error');
    assert((await cdp.eval('document.querySelector(' + JSON.stringify(box) + ').textContent')).includes('不要合并'));
    mode = 'success'; evidence.checks.push('Connection loss shown in status row; recognized text stays directly editable');
    ...
    await until('document.querySelector("#mr-input-box").textContent.includes("不要合并")', 'group recording');
    ...
    assert.equal(await cdp.eval('document.querySelectorAll("#meeting-room-panel .voice-panel").length'), 0);
    assert.equal(await cdp.eval('!!document.querySelector("#mr-composer-head .voice-status")'), true);
    ...
    await click('#mr-input-row .voice-mic'); await until('document.querySelector("#mr-input-box").textContent.endsWith("不要合并")', 'switch recording');
    await cdp.eval('selectMeeting(' + JSON.stringify(g2.id) + ')');
    await draft('#mr-input-box', '另一群的草稿');
    await until('document.querySelector("#mr-input-row .voice-mic").textContent === "语音"', 'switch protected');
    await sleep(500);
    assert.equal(await cdp.eval('document.querySelector("#mr-input-box").textContent'), '另一群的草稿');
    await cdp.eval('selectMeeting(' + JSON.stringify(g.id) + ')');
    assert((await cdp.eval('document.querySelector("#mr-input-box").textContent')).endsWith('不要合并'));
    evidence.checks.push('Switching groups stops capture; late results never enter another group; original group retains live draft');
    assert(evidence.audio.length >= 3); assert(evidence.audio.every(a => a.bytes > 0 && a.bytes % 2 === 0 && a.sampleRate >= 8000));
