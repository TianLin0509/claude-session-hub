...
任务推进到一个可解释的新阶段、遇到阻碍或改变验证安排时，主动在群聊写一行：

`UPDATE: 用中文说明刚完成什么、现在在做什么或被什么挡住`

这是一条过程进展，开发工作台会随群聊消息同步呈现。不要为了刷进度重复空话，也不要为更新而额外调用模型。
最终交接前不要提前输出下面的 `PROGRESS` 四行；`UPDATE` 不表示完成，不代替最终汇报。
